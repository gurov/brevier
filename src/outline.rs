...
/// Масштаб страницы ступенями, как в браузере. Лестница знакомая —
/// в неё попадают не думая, — и ступени редкие: разница меньше десятой
/// доли на экране не читается, а длинный список начинает требовать памяти.
///
/// Множитель один на всю модель разом: кегль, мера, шкала заголовков,
/// отступы списка, поля блока кода, линейка цитаты. Порознь они разъедутся
/// между собой. Мера задана в кеглях, поэтому строка на любой ступени
/// остаётся той же длины в знаках — ровно это и делает браузер, увеличивая
/// страницу, а не один шрифт.
pub const ZOOM_STEPS: [f32; 9] = [0.67, 0.8, 0.9, 1.0, 1.1, 1.25, 1.5, 1.75, 2.0];
/// Ступень «как задумано»: к ней возвращает Ctrl+0.
pub const ZOOM_NORMAL: usize = 3;
...
он занял несколько строк на нашей мере и попал в разметку вехой.";
    ...
    /// Ctrl+0 обязан возвращать ровно ту типографику, которая выбрана
    /// в остальных константах, — иначе «сто процентов» ничего не значит.
    #[test]
    fn the_zoom_ladder_stands_on_the_size_we_chose() {
        assert_eq!(ZOOM_STEPS[ZOOM_NORMAL], 1.0);
